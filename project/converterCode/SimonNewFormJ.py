...
def accDiff(x1,x2,t1,t2):
    try:
	return (x2-x1)/(t2-t1).total_seconds()
    except ZeroDivisionError:
	print 'acc:' + str(t2-t1)
	return 'ZeroError'
    except :
...
def accelerations(x,t):
    assert(isinstance(t[0],datetime))
    assert(isinstance(x[0],float))
    return [accDiff(x1,x2,t1,t2) for x1,x2,t1,t2 in zip(x[:-1],x[1:],t[:-1],t[1:])]
...
for trip in data:
    ...
    print V[0]
    print  WO + 'time'
    T = [datetime.strptime(t, '%Y-%m-%d %H:%M:%S') for v,t in trip]
    print T[0]
    print WO + 'distance'
    D = distances(V,T)
    D = D + [D[-1]]
    ...
    trip2 = [[d,v,a,t.strftime('%Y-%m-%d %H:%M:%S')] for d,v,a,t in zip(D,V,A,T) if a<>'ZeroError']
    data2.append(trip2)
    i+=1
